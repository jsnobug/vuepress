...
转至：https://juejin.cn/post/6872111128135073806

转至：https://ts.xcatliu.com/
...
在声明函数时，可以通过 `?` 号来定义可选参数，比如 `age?: number` 这种形式。**在实际使用时，需要注意的是可选参数要放在普通参数的后面，不能放到默认参数后面，不然会导致编译错误**。
...
![typescript1](/imgs/typescript1.image)
...
![typescript1](/imgs/typescript2.image)
